...
def bffm2(aircraft_data, altitude, mach, W_f):
    ...
    fuel_flow_icao = np.array([aircraft_data['Fuel Flow T/O (kg/sec)'],
                            aircraft_data['Fuel Flow C/O (kg/sec)'],
                            aircraft_data['Fuel Flow App (kg/sec)'],
                            aircraft_data['Fuel Flow Idle (kg/sec)']]).reshape(1,-1)
    
    HC_ei_vec = np.array([aircraft_data['HC EI T/O (g/kg)'],
                          aircraft_data['HC EI C/O (g/kg)'],
                          aircraft_data['HC EI App (g/kg)'],
                          aircraft_data['HC EI Idle (g/kg)']]).reshape(1,-1)
    
    CO_ei_vec = np.array([aircraft_data['CO EI T/O (g/kg)'],
                        aircraft_data['CO EI C/O (g/kg)'],
                        aircraft_data['CO EI App (g/kg)'],
                        aircraft_data['CO EI Idle (g/kg)']]).reshape(1,-1)
    
    NOx_ei_vec = np.array([aircraft_data['NOx EI T/O (g/kg)'],
                           aircraft_data['NOx EI C/O (g/kg)'],
                           aircraft_data['NOx EI App (g/kg)'],
                           aircraft_data['NOx EI Idle (g/kg)']]).reshape(1,-1)

    atmosphere = Atmosphere(altitude)
    P_amb = pa_to_psi(atmosphere.pressure)
    T_amb = atmosphere.temperature
    ...
    fuel_flow_evaluate = np.log10(W_f)
